...
Route user input through `ui::events` and typed messages, then through screen handling and `Effect` application. Do not add ad hoc key handling inside render code. Treat `ui::store` as the first data port: UI code should ask the store for data instead of reaching into `SessionIndex` or future index engines directly. Shared UI state should move through the reusable reactive-store pattern (`dispatch` action, reducer transition, selector read) so refreshes can reconcile state instead of resetting navigation or panels.

Model long-lived TUI data in `ui/app` before rendering it. `AppState` owns domain slices for project loading, files, graph/index state, search, Git changes, blast-radius impact, panels, and future coverage. Its work catalog names the asynchronous candidates (`ProjectLoad`, `FileCatalog`, `GraphIndex`, `SearchIndex`, `GitChangeIndex`, `ImpactIndex`, `PanelData`, `CoverageIndex`) so future lazy loading can schedule work explicitly. Add new cross-panel state there first, then expose it through store selectors or effects; do not hide durable state inside one panel renderer.
