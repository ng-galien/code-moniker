...
The srcset is the boundary node. Its kind is supplied by the caller (`srcset`, `workspace_app`, etc.). Below it, the **first segment of the language regime is `lang:<short>`**, posted by the extractor (`lang:ts`, `lang:rs`, `lang:java`, `lang:python`, `lang:go`, `lang:cs`, `lang:sql`). The extension does not interpret srcsets — it receives the srcset moniker as an anchor and emits all source-derived nodes under it, prefixed by the language segment.
...
- **Per-language extractors** : `extract_typescript(...)`, `extract_rust(...)`, `extract_java(...)`, `extract_python(...)`, `extract_go(...)`, `extract_csharp(...)`, `extract_plpgsql(...)`. One per supported language.
...
Phases 1–7 shipped: typed URI, `moniker` and `code_graph` types with custom Datum layout, btree / hash / GiST / GIN, compact projection, seven extractors (TS, Rust, Java, Python, Go, C#, PL/pgSQL), six manifest parsers, dogfood panel, and Phase 7 cross-file linkage (`bind_match` + `binding` + `lang:` segment). A post-7 chantier added the declarative graph constructor (`code_graph_declare` / `code_graph_to_spec`) and formalised the per-language extractor contract (`lang::LangExtractor` trait + `define_languages!` macro + `assert_conformance`).

### Phase 7 — Cross-file linkage

Three coordinated changes unlock cross-file linkage:
...
- **Go** — imports (`import "github.com/x/y/z"`) lower to `external_pkg:<first piece>/path:<rest>` in the project regime (no `lang:go/` prefix on the import target). The default bind name is the path's last piece; `import alias "..."` overrides it; `import _ "..."` and `import . "..."` skip the bind. Stdlib paths (`fmt`, `os`, …) keep the same shape.
- **C#** — `using A.B.C` lowers to `external_pkg:A/path:B/path:C` in the project regime (same flat shape as Go). BCL prefixes (`System`, `Microsoft`, `mscorlib`) carry `confidence=external`; everything else carries `confidence=imported`. Aliases (`using X = ...`) and `static` / `global` flavours are surfaced via `RefAttrs` (`alias`, `binding`).
- **SQL / PL-pgSQL** — all `calls` refs are tagged `binding=local` (intra-module by language design) and use arity-only callable names like `function:bar(2)`, while defs use typed names like `function:bar(int4,text)`. `bind_match`'s `lang:sql` arm matches on the bare callable name (segment kind + name before the `(`), so cross-file linkage works without a separate refinement layer.

`bind_match`'s last-segment compare is dispatched per `lang:` segment in `core/moniker/query.rs::last_segment_match`. Current arms: `sql`, `ts`, `java`, `python`, `rs`, `go` — every one collapses on `bare_callable_name`. Languages without an arm fall back to byte-strict last-segment-name equality.

Validation snapshot (Phase-7 ship): cross-file linkage on the dogfood panel resolves at scale across the four languages whose extractors were present at ship. Numbers are a snapshot — Go, C#, and the SQL `lang:sql` arm shipped after this measurement.
...
- Alias-aware refinements of `bind_match` beyond the bare-callable-name arm shared by every supported language. The current per-`lang:` dispatch is byte-strict on the segment kind and collapses signatures via `bare_callable_name`; richer alias rules (e.g. TS rename re-exports) remain open.
