...
use crate::workspace::code::{NormalizedSource, NormalizedSymbol, SymbolProvider};
...
pub(crate) struct ChangeAnalyzer<'a, P> {
	symbols: &'a P,
}
...
where
	P: SymbolProvider,
{
	pub(crate) fn new(symbols: &'a P) -> Self {
		Self { symbols }
	}

	pub(crate) fn analyze(&self, entries: &[diff::ChangeEntry]) -> Vec<ChangeRecord> {
		entries
			.iter()
			.enumerate()
