...
use crate::workspace::code::{def_kind, is_navigable_def, last_name};
...
pub(crate) struct ChangeEntry {
	pub(crate) loc: Option<SymbolLocation>,
...
impl ChangeIndex {
	pub(crate) fn entry_for(&self, loc: &SymbolLocation) -> Option<&ChangeEntry> {
		self.entries_by_symbol
			.get(loc)
			.and_then(|idx| self.entries.get(*idx))
	}

	pub(crate) fn changed_symbols(&self) -> Vec<SymbolLocation> {
...
mod tests {
	...
	fn build_change_index_reports_modified_symbols() {
		...
		let ctx = crate::extract::Context::default();
		let graph = crate::extract::extract(Lang::Rs, source, Path::new("src/lib.rs"));
