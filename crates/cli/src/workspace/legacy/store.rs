...
use crate::perf;
use crate::sources;
use crate::workspace::code::{compact_moniker, def_kind, is_navigable_def, last_name, ref_kind};
use crate::workspace::compat::SessionStoreBridge;
use crate::workspace::git::{ChangeEntry, ChangeFile, ChangeIndex, ChangeRoot, ChangeScan};
use crate::workspace::snapshot::SymbolLocation;
...
fn build_change_index(index: &SessionIndex) -> ChangeIndex {
	...
	let files = index
		...
		.collect();
	crate::workspace::git::build_change_index(ChangeScan { roots, files })
}
	...
	for change in &change_index.entries {
		cache.entry(change.moniker.clone()).or_insert_with(|| {
			let refs = change.loc.map_or_else(
				...
				|loc| linkage.incoming_refs_for_def(&def_location_from_symbol_location(loc), index),
			);
			refs.into_iter()
...
fn symbol_location_from_def_location(loc: &DefLocation) -> SymbolLocation {
	SymbolLocation {
		file: loc.file,
		symbol: loc.def,
	}
...
fn def_location_from_symbol_location(loc: SymbolLocation) -> DefLocation {
	DefLocation {
		file: loc.file,
		def: loc.symbol,
	}
...
impl IndexStore for WorkspaceStore {
	...
	fn changed_defs(&self) -> Vec<DefLocation> {
		self.snapshot
			...
			.changed_symbols()
			.into_iter()
			.map(def_location_from_symbol_location)
			.collect()
	}
	...
	fn change_detail_for_symbol(&self, loc: &DefLocation) -> Option<ChangeDetail> {
		self.snapshot
			.git
			.change_index
			.entry_for(&symbol_location_from_def_location(loc))
