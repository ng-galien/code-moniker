...
use crate::workspace::session::{ChangeId, ChangeRecord, ChangeRecordCoreFields, ChangeStatus};

pub(super) struct ChangeAnalyzer<'a, P> {
	symbols: &'a P,
}
...
where
	P: SymbolProvider,
{
	pub(super) fn new(symbols: &'a P) -> Self {
		Self { symbols }
	}

	pub(super) fn analyze(&self, entries: &[git::ChangeEntry]) -> Vec<ChangeRecord> {
		entries
			.iter()
			.enumerate()
...
fn change_status(status: git::ChangeStatus) -> ChangeStatus {
	match status {
		git::ChangeStatus::Added => ChangeStatus::Added,
		git::ChangeStatus::Modified => ChangeStatus::Modified,
		git::ChangeStatus::Removed => ChangeStatus::Removed,
	}
}

impl From<&git::ChangeEntry> for PendingChange {
	fn from(entry: &git::ChangeEntry) -> Self {
		Self {
			loc: entry.loc,
