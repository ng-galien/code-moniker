...
use crate::workspace::resources::symbol_provider::{
...
use crate::workspace::resources::{
	...
	LocalCodeIndexOptions, LocalIdentityResolver, LocalLinkage, LocalResourceCache,
	LocalSourceCatalog, LocalSourceCatalogOptions,
};
use crate::workspace::session::{
	CodeIndexPort, LinkagePort, SourceCatalogPort, WorkspaceRequest, WorkspaceSession,
...
fn local_check_runner_counts_rule_severity() {
	...
	let mut check_runner = LocalCheckRunner::new(fixture.check_options(), cache);
...
fn local_check_runner_maps_symbols_with_custom_scheme() {
	...
	let mut index_port = LocalCodeIndex::new(LocalCodeIndexOptions::default(), cache.clone());
	let mut linkage_port = LocalLinkage::new(cache.clone());
	let mut check_runner = LocalCheckRunner::new(
		LocalCheckRunnerOptions::new(fixture.rules.clone(), None, "custom+moniker://"),
		cache,
	);
...
impl SymbolProvider for EmptySymbolProvider {
	...
	fn symbol_at(&self, _loc: DefLocation) -> Option<NormalizedSymbol> {
		None
	}
...
"#,
	...
	fn check_options(&self) -> LocalCheckRunnerOptions {
		LocalCheckRunnerOptions::new(self.rules.clone(), None, crate::DEFAULT_SCHEME)
	}
	...
	fn workspace_session(
		&self,
		cache: LocalResourceCache,
	) -> WorkspaceSession<LocalSourceCatalog, LocalCodeIndex, LocalLinkage, LocalChangeOverlay> {
