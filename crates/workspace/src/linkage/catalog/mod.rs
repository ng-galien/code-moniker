mod candidate;
mod ordinals;
mod query;

pub(in crate::linkage) use crate::snapshot::{SymbolOrdinal, SymbolOrdinalCatalog, SymbolSet};
pub(in crate::linkage) use candidate::{CandidateCatalog, LinkageCandidate, query_keys};
pub(crate) use ordinals::{ReferenceOrdinal, ReferenceSet, ReferenceSetIter};
pub(in crate::linkage) use query::{LinkageQuery, ReferenceLocation, ReferenceLocations};
