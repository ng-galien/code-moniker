...
int included_value(void) {
	...
	return DOUBLE(3) + DOUBLE(1, 2) + VARIADIC(1, 2, 3) + MATH_VERSION + MATH_MODE_FAST + TOKEN_KIND(FAST) + MIXED(FAST, mixed_typo) + ordinary_typo + injected_value + field_value + record.value + buffer_ptr->len;
}
