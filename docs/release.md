...
- [ ] Push `v0.9.1` only after the preceding gates pass.
...
- [ ] Confirm the Release workflow completes through `announce`, all seven
      crates exist on crates.io, and all five packages exist on npm at `0.9.1`.
- [ ] On clean macOS, Linux and Windows environments, exercise the direct
      installer and `cargo binstall code-moniker --version 0.9.1`.
...
- [ ] Confirm clean ESM and CommonJS consumers install
      `@code-moniker/client@0.9.1` and receive the matching native package.
