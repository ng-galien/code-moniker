...
## `0.8.1` acceptance checklist

- [ ] `main` is clean, the six release-gate CI jobs listed above are green, and
      no `v0.8.1` tag exists. The `vscode-extension` job is not part of this
      gate.
- [ ] All workspace crates that are published share version `0.8.1`.
- [ ] `dist plan --tag=v0.8.1` lists exactly the five supported targets,
...
- [ ] Push `v0.8.1` only after the preceding gates pass.
...
- [ ] Confirm the Release workflow completes through `announce`, all seven
      crates exist on crates.io, and all five packages exist on npm at `0.8.1`.
- [ ] On clean macOS, Linux and Windows environments, exercise the direct
      installer and `cargo binstall code-moniker --version 0.8.1`.
...
- [ ] Confirm clean ESM and CommonJS consumers install
      `@code-moniker/client@0.8.1` and receive the matching native package.
