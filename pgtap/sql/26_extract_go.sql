...
SELECT is(graph_root(g)::text,
	'code+moniker://app/lang:go/package:acme/package:util/module:text',
...
SELECT
	is((SELECT visibility FROM graph_defs(g) d
	     WHERE kind = 'func' AND
	           moniker = 'code+moniker://app/lang:go/module:m/func:Hello()'::moniker),
		'public',
		'capitalized name is public') AS r4,
	is((SELECT visibility FROM graph_defs(g) d
	     WHERE kind = 'func' AND
	           moniker = 'code+moniker://app/lang:go/module:m/func:helper()'::moniker),
		'module',
...
SELECT
	ok(g @> 'code+moniker://app/external_pkg:net/path:http/func:Get(1)'::moniker IS NOT NULL,
