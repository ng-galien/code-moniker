...
title: Executable architecture patterns
lang: java
blurb: Browse architecture boundaries, Clean Architecture, Fowler patterns, and package isolation
learn_kind: pattern
learn_path: architecture/patterns
learn_order: 10
tags: java,mvc,ddd,hexagonal,architecture
learn_aliases: mvc,ddd,hexagonal
published: true
---
...
One source line can emit several distinct reference kinds (for example an
import, type use, instantiation, or call). Because the scenario expectation
format records rule and range rather than the relation kind, such findings can
look identical below even though the graph evaluates separate edges.
