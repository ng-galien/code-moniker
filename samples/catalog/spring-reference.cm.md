---
name: spring-reference
title: Spring complete reference pack
lang: java
blurb: A comprehensive opt-in pack after the focused Spring topics have been reviewed
learn_kind: framework
learn_path: languages/java/spring/reference
learn_order: 90
tags: java,spring,spring-aop,junit,mvc,persistence,transactions
published: true
---

# Spring complete reference pack

This opt-in pack demonstrates executable Spring signals over a small
Maven-style project: direct stereotype annotations, layer boundaries,
transaction placement, proxy self-invocation risks, and test slices. It also
contains a few Java and team-policy guardrails used by the fixture.

These checks are structural heuristics, not a semantic Spring model. Annotation
matching uses simple names on direct `annotates` references: composed
stereotypes are not expanded. Repository checks target classes directly
annotated `@Repository`; an unannotated Spring Data interface extending
`Repository`, `CrudRepository`, or `JpaRepository` is outside this pack.
Injection checks cover fields directly annotated `@Autowired`, not `@Inject`,
`@Resource`, constructor parameters, or generated wiring. Adapt or remove each
policy to match the project before enabling it.
...
rationale = "This is an example team policy, not a JUnit requirement. Some teams keep a test prefix while others prefer descriptive names or display-name generation."
expr = """
  $java_test
...
rationale = "In proxy mode, a call through `this` can bypass a second interception. This broad class-level signal is intentionally opt-in because many internal calls do not require another transaction or advice invocation, and AspectJ weaving has different semantics."
expr = """
  any(out_refs,
    kind = 'annotates'
    AND target.name =~ ^(Transactional|Async|Cacheable|CachePut|CacheEvict|Caching|Retryable|ConcurrencyLimit|PreAuthorize|PostAuthorize|PreFilter|PostFilter|Secured|RolesAllowed)$
...
"""
message = "Audit same-class calls in proxied Spring class `{name}` when the call expects another interception."
...
Domain code reaches into infrastructure through an explicit import. The
minimal source keeps this one diagnostic distinct from the Spring checks:
...
## Spring annotation provenance

The Spring recipes below select annotations by simple target name so they also
work when external package provenance is unavailable. That is a documented
heuristic: an application-defined annotation named `Service`, `Transactional`,
or `Autowired` can match it. Projects with such homonyms should add a target
package or `external_pkg` predicate to the aliases before adopting the pack.
...
outside the service layer, one direct method-level proxy bypass, and one broad
class-level audit signal. The latter matters only when the internal call is
expected to trigger another interception:
...
A package-visible JUnit method that does not start with `test`, two badly
suffixed slice tests, one badly suffixed full-context test, and a test mixing
`@SpringBootTest` with `@WebMvcTest`:
