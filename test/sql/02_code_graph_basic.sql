...
WITH g0 AS (
	...
	SELECT graph_add_def(
		g,
		'esac+moniker://app/path:main/path:Foo/class:Foo'::moniker,
		'class',
		'esac+moniker://app/path:main/path:Foo'::moniker
	) AS g FROM g0
), g2 AS (
	SELECT graph_add_def(
		g,
		'esac+moniker://app/path:main/path:Foo/class:Foo/method:bar()'::moniker,
		'method',
		'esac+moniker://app/path:main/path:Foo/class:Foo'::moniker
	) AS g FROM g1
), gref AS (
	SELECT graph_add_ref(
		g,
		'esac+moniker://app/path:main/path:Foo/class:Foo/method:bar()'::moniker,
		'esac+moniker://app/path:main/path:Bar/class:Bar'::moniker,
		'call'
	) AS g FROM g2
)
SELECT
	is(graph_root(g)::text, 'esac+moniker://app/path:main/path:Foo',
		'graph_root returns the constructor''s root') AS r1,
	ok(g @> 'esac+moniker://app/path:main/path:Foo'::moniker,
		'graph contains its root') AS r2,
	ok(g @> 'esac+moniker://app/path:main/path:Foo/class:Foo'::moniker,
		'graph contains an added def') AS r3,
	ok(g @> 'esac+moniker://app/path:main/path:Foo/class:Foo/method:bar()'::moniker,
		'graph contains a nested def') AS r4,
	ok(NOT (g @> 'esac+moniker://app/path:main/path:Bar/class:Bar'::moniker),
